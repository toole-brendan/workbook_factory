...
Replaces the separate TAM and SAM executive summaries with one front door:
addressable TAM, observed first-tier reported SAM, a TAM-to-SAM bridge, and
the highest-value where-to-play / hull-visibility callouts.
"""
...
from workbook_master_tam.sheets import ddg_tam as TAM
...
from workbook_award_classification_refactor.sheets.supplier_year_activity import supplier_year_cols
...
from workbook_award_classification_refactor.sheets.ddg_hull_spend_summary import hull_spend_cols
...
def _render() -> WorksheetSpec:
    ...
    c.write(["SAM", "Observed reported first-tier hull-builder subawards"],
            styles=[S_BOLD, S_DEFAULT])
    c.write(["Bridge caveat", "Observed SAM / TAM is a reporting-and-reach bridge, not full market penetration."],
            styles=[S_BOLD, S_DEFAULT])
    c.blank(2)
    ...
    c.blank()
    c.write(["FY", "TAM $M", "Observed SAM $M", "Observed SAM / TAM"],
            styles=[S_HEADER_LEFT] * 4)
    ...
    c.write(["Bridge uses reported first-tier subawards only; exclusions and retained in-house scope remain outside observed SAM."],
            styles=[S_ITALIC])
    c.blank(2)
...
EXECUTIVE_SUMMARY = SheetEntry(TAB_NAME, _GROUP, _render)

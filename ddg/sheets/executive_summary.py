...
def _render() -> WorksheetSpec:
    ...
    c.write(["SAM", "Observed reported first-tier hull-builder subawards"], styles=[S_BOLD, S_DEFAULT])
    c.write(["Bridge caveat", "Observed SAM / TAM is a reporting-and-reach bridge, not full market penetration."], styles=[S_BOLD, S_DEFAULT])
    c.blank(2)
    ...
    c.write(["Exact-hull assigned $M", f"={assigned}", "A/B rows assigned to a single hull; C/D family-level dollars are not allocated."], styles=[S_DEFAULT, S_LINK_NUM, S_DEFAULT])
    c.write(["Exact-hull share of observed SAM", f'=IFERROR({assigned}/{lifetime},"")', "Share of total DDG observed SAM that is exact-hull attributable."], styles=[S_DEFAULT, S_PCT, S_DEFAULT])

...
_FY_COL_RE = re.compile(r"^(?:≤FY|FY)(\d+)\b")


def _fy_col_year(header: str) -> int | None:
    """The federal FY a wide-format 'FYxx $M' / '≤FY12 $M' column represents, or None
    if it isn't an FY-bucketed column."""
    m = _FY_COL_RE.match(header)
...
def load_table(name: str) -> tuple[list[str], list[list[str]]]:
    ...
    rows = _filter_sam_window(name, headers, rows)
    if name == "ddg_swbs_by_subsystem":
        headers, rows = _trim_fy_columns(headers, rows)
    return headers, rows

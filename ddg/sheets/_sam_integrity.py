...
from sheets._sam_cuts import load_table
from sheets._data import sam as _data_path
...
# Versioned prime-contract scope manifest (data/workbook_inputs/sam_awards/scope/).
_SCOPE_MANIFEST = _data_path("prime_contract_scope")
...
def assert_archetype_codes_valid() -> None:
    ...
    from sheets import _sam_taxonomy as _taxonomy
...
def assert_universes_aligned() -> None:
    """Fail the build unless the transaction fact spine and Supplier Master describe the
    SAME (Program x UEI) universe - the two CSVs the workbook actually reads at runtime.
    (The program-vendor roll-up is now an in-memory cut of Supplier Master, no longer a
    CSV input, so it is no longer part of this guard - comparing the real runtime sources
    catches the same refresh-drift it used to.)"""
    tx = _single_program_keys("subaward_transactions")
    sm = _dimension_keys("supplier_master")

    assert tx == sm, "transaction vs Supplier Master (Program x UEI) drift: " + _diff(
        tx, sm, "transaction", "Supplier Master")
...
def assert_transaction_dates_covered_by_fiscal_axis() -> None:
    ...
    from sheets._sam_fiscal import FY_BASE, FY_START

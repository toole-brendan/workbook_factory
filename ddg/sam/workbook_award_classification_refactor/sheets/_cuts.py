"""DDG SAM CSV accessors.

Historical SAM sheet modules still request legacy CSV stems such as
``supplier_master``.  The files now live in the consolidated DDG data tree
with explicit DDG/reference names; ``ddg.lib`` owns that mapping.
"""
...
from ddg.lib import resolve_sam_csv

_EPOCH = date(1899, 12, 30)          # Excel date serial day 0

def load_grid(name: str) -> list[list[str]]:
    """Full row-major grid for a DDG SAM workbook-input CSV as raw strings."""
    with resolve_sam_csv(name).open(encoding="utf-8", newline="") as fh:
...
def load_headers(name: str) -> list[str]:
    """Just the header row of a DDG SAM workbook-input CSV."""
    with resolve_sam_csv(name).open(encoding="utf-8", newline="") as fh:
...
def cy_span(stem: str, date_header: str = "Subaward Date") -> str:
    """'CY<min>-<max>' calendar-year span of a transaction CSV's date column."""
...
def cy_span_union(stems: list[str], date_header: str = "Subaward Date") -> str:
    """'CY<min>-<max>' across several transaction CSVs."""
...
def date_serial(s):
    """ISO date text -> Excel date serial (None for blank)."""
    if not s:
        return None
